...
def process_pdf(in_file, replacements):

    filename_base = in_file.replace(os.path.splitext(in_file)[1], "")

    # Provide replacements list that you need here
    #replacements = { '[[name]]': 'Graham'}

    pdf = PdfReader('./static/pdf_templates/' + in_file + '.pdf')
    writer = PdfWriter()
    ...
    with open('./static/pdf_generated/' + in_file + '_out' + '.pdf', 'wb') as out_file:
